// session_class file
//based on https://www.boost.org/doc/libs/1_36_0/doc/html/boost_asio/example/echo/async_tcp_echo_server.cpp
...
void session::handle_read(const boost::system::error_code& error,
  ...
  if (!error) {
    ...
    if (result == request_parser::good) //valid URL
    { 
      printf ("200: %c %c \n", 'a', 65);
    ...
    else if (result == request_parser::bad) // bad URL
    {
      printf ("400: %c %c \n", 'a', 65);
